...
namespace WiredBrainCoffee.CupOrderAdmin.Core.Services.OrderCreation
{
    public class OrderCreationService : IOrderCreationService
    {
        ...
        public OrderCreationService(IOrderRepository orderRepository, ICoffeeCupRepository coffeeCupRepository)
        {
            _orderRepository = orderRepository ?? throw new ArgumentNullException(nameof(orderRepository));
            _coffeeCupRepository = coffeeCupRepository ?? throw new ArgumentNullException(nameof(coffeeCupRepository));
        }

        public async Task<OrderCreationResult> CreateOrderAsync(Customer customer, int numberOfOrderedCups)
        {
            if (customer == null) throw new ArgumentNullException(nameof(customer));

            if (numberOfOrderedCups < 1) throw new ArgumentOutOfRangeException(nameof(numberOfOrderedCups), $"{nameof(numberOfOrderedCups)} must be greater than zero");
        ...
        private static double CalculateDiscountPercentage(CustomerMembership membership, int numberOfOrderedCups)
        {
        ...
        private async Task<Order> CreateOrderInternalAsync(Customer customer, int numberOfOrderedCups)
        {
